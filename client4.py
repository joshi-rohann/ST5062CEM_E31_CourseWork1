...
class Client:
    def __init__(self, host, port):
        ...
        self.msg = tkinter.Tk("")
        self.msg.title("Nickname")
        self.msg.resizable(0, 0)

        self.nname = tkinter.StringVar()

        label = tkinter.Label(
            self.msg, text="Welcome to the chat room!", font=("Helvetica", 16))
        label.pack()

        entry_label = tkinter.Label(
            self.msg, text="Enter a nickname of your choice: ")
        entry_label.pack()

        entry = tkinter.Entry(self.msg, textvariable=self.nname)
        entry.pack()

        submit_btn = tkinter.Button(
            self.msg, text="Submit", command=self.get_nickname)
        submit_btn.pack()
    ...
    def get_nickname(self):
        self.nickname = self.nname.get()

    def gui_loop(self):
        self.win = tkinter.Tk()
        self.win.configure(bg="#83838B")
        self.win.title(f"Chat of {self.nname.get()}")
